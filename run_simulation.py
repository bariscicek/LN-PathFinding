...
import networkx.algorithms.shortest_paths.astar as nx2
...
def callable(source, target, amt, result, name):
    ...
    def dijkstra_caller(res_name, func):
        dist = nx2(G, source=target, target=source, heuristic=None, weight = func, pred=prev_dict, paths=paths)
        res = paths[source]
